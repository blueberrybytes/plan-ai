...
Plan AI is a private AI meeting assistant for software agencies, engineering teams and Technical Project Managers (TPMs). It turns engineering meetings into tickets, specs and diagrams, and it is built so that confidential meetings stay confidential.
...
Engineering meetings carry client code, architecture decisions and plans that are not public yet. Plan AI treats them that way:

- **Private by default.** Recordings, voice profiles and files are never public. Every read goes through a signed link that expires.
- **Shared only on purpose.** Meeting documents and slides stay inside your workspace until you share them, and you can stop sharing at any moment.
- **AI on your own servers.** With a private install, transcription, speaker identification, the language model and search run on your infrastructure. No meeting content goes to an AI provider.

The [Security Overview](/security/overview) says where each part runs, on the cloud and in a private install.
...
1. **Record:** You record your client syncs with the app on your own machine, with no bot in the call.
...
Our native macOS and Windows desktop applications capture the system audio directly from your soundcard. The client never sees a bot and the meeting is never interrupted.
...
Plan AI is built as a monorepo with three applications that work together:
