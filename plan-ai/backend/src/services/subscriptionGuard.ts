...
export class SubscriptionRequiredError extends Error {
  ...
  constructor(
    ...
    public reason: "no_subscription" | "expired" | "canceled" | "incomplete" | "over_quota" = "no_subscription",
  ) {
    super(message);
...
export const requireActiveSubscription = async (workspaceId: string): Promise<void> => {
  ...
  if (!result.active) {
    ...
    if (result.reason === "over_quota") {
      message = "You have more members than paid seats. Please remove members or upgrade your subscription to continue.";
    }
