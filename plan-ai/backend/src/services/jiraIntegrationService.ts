...
class JiraIntegrationService {
    ...
    if (boardRes.status === "fulfilled") {
      if (boardRes.value.ok) {
        ...
        if (boardRes.value.status === 401 || boardRes.value.status === 403) {
          logger.warn(
            "Jira board endpoint failed (likely missing read:board-scope:jira-software scope)",
            {
  ...
  public async createJiraIssue(
    ...
    const sanitizedProjectName = task.project.title
      .replace(/\s+/g, "-")
      .replace(/[^a-zA-Z0-9-]/g, "");
    ...
    const postIssue = async (
      ...
      { ok: true; data: { id: string; key: string } } | { ok: false; status: number; body: string }
    > => {
    ...
    // Browser-facing link: must use the customer's site domain
    // (https://acme.atlassian.net). For OAuth, `cleanUrl` is the
    // api.atlassian.com gateway — valid for REST calls but its /browse
    // redirects to a login page that 404s. jiraSiteUrl is stored for both
    // auth types at connect time (from accessible-resources).
    const browseBase = (meta?.jiraSiteUrl || cleanUrl).replace(/\/$/, "");
    return {
      issueId: result.data.id,
      issueKey: result.data.key,
      url: `${browseBase}/browse/${result.data.key}`,
    };
  }
