...
class ContextService {
  ...
  public async removeFileFromContext(
    ...
    // Best-effort: drop the per-repo GitNexus index ("ladybug") tied to this
    // file. Doing it here — not in the controller — also covers the github
    // worker's pre-emptive cleanup on re-sync, so stale indexes never leak.
    ladybugService.deleteLadybug(file.metadata).catch(() => {});
