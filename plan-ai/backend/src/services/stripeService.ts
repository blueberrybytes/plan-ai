 ...
 */
/**
 * Free-trial length (in days) applied to BYOK checkouts. Returns 0 when trials
 * are disabled (env unset to 0 or invalid). MANAGED plans never get a trial.
 * Single source of truth for both the checkout (trial_period_days) and the
 * pricing UI ("N-day free trial"), so the two can't drift.
 */
export const getByokTrialDays = (): number => {
  const days = Number(EnvUtils.get("STRIPE_BYOK_TRIAL_DAYS", "14"));
...
export const createCheckoutSession = async (params: {
  ...
  const descriptor = catalog[priceId];
  if (!descriptor) {
    throw new Error(`Price ID ${priceId} not found in catalog. Check STRIPE_PRICE_* env vars.`);
  }

  // Free trial — only for BYOK plans. With BYOK the customer brings their own
  // AI keys, so a trial costs us nothing; MANAGED plans would have us eating the
  // AI spend during the trial, so they never get one. Length is configurable;
  // set STRIPE_BYOK_TRIAL_DAYS=0 to disable. The card is still collected upfront
  // (Stripe Checkout's default), so Stripe auto-charges when the trial ends.
  const byokTrialDays = getByokTrialDays();
  const trialPeriodDays =
    descriptor.track === "BYOK" && byokTrialDays > 0 ? byokTrialDays : undefined;
 ...
 */
export const retrieveSubscription = async (subscriptionId: string): Promise<StripeSubscription> => {
  const stripe = getStripe();
