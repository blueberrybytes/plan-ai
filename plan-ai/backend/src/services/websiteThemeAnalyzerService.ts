...
export class WebsiteThemeAnalyzerService {
  ...
  private extractFontFamilies(css: string): string[] {
    ...
    while ((match = FONT_FAMILY_RE.exec(css)) !== null) {
      const raw = match[1]
        ...
        .filter((f) => !["sans-serif", "serif", "monospace", "cursive", "system-ui", "inherit", "initial", "-apple-system", "BlinkMacSystemFont", "Segoe UI"].includes(f));
  ...
  public async analyzeUrl(url: string, workspaceId: string) {
    ...
    const fetchPromises = externalStylesheetUrls.slice(0, 3).map((href) => this.fetchExternalCss(href));
    ...
    const boringColors = new Set(["#fff", "#ffffff", "#000", "#000000", "#0000", "rgb(0, 0, 0)", "rgb(255, 255, 255)", "rgba(0, 0, 0, 0)", "rgba(255, 255, 255, 0)"]);
...
${Object.keys(colorVars).length > 0 ? Object.entries(colorVars).map(([k, v]) => `${k}: ${v}`).join("\n") : "(none found)"}
