...
export class GithubIntegrationService {
  ...
  private bindWebhookListeners() {
    ...
    this.webhooks.on("push", async ({ id, payload }) => {
      ...
      try {
        ...
        Sentry.captureException(err, { tags: { event: "github_push", deliveryId: id } });
      }
    });
  ...
  private async handlePushEvent(payload: {
    ...
    ref?: string;
    /** SHA of the head commit after the push — used to key job dedupe. */
    after?: string;
  }) {
    ...
    // Derive the branch name from the ref (e.g. "refs/heads/main" → "main")
    const pushedBranch = payload.ref?.replace("refs/heads/", "") ?? "HEAD";
    ...
    logger.info(
      `[GitHub Push] ${repoFullName}@${pushedBranch} — re-syncing linked project knowledge bases`,
    );

    // --- Step 1: Re-sync all project knowledge bases linked to this repo + branch ---
    // Find every Context whose metadata.githubRepoFullName matches the pushed repo
    // AND whose githubBranch matches the pushed branch (or is unset, meaning HEAD).
    // We cast metadata to any to do a safe JSON field lookup.
    try {
      const linkedContexts = await prisma.context.findMany({
        where: {
          metadata: {
      ...
      for (const ctx of linkedContexts) {
        ...
        const savedBranch = (meta.githubBranch as string | undefined) ?? "HEAD";

        // Only re-sync if the push is to the branch this context is tracking
        if (savedBranch !== pushedBranch && savedBranch !== "HEAD") continue;
        ...
        await githubContextQueue.add(
          ...
          {
            // Deduplicate per HEAD COMMIT, not per context+repo+branch: BullMQ
            // also dedupes against COMPLETED jobs still in history
            // (removeOnComplete keeps 100), so a static jobId made every push
            // after the first one a silent no-op. Keying by `after` sha keeps
            // the useful dedupe (GitHub redeliveries of the same push) while
            // letting every new push enqueue a fresh resync.
            jobId: `push-resync-${ctx.id}-${(payload.after ?? "").slice(0, 12) || Date.now()}`,
          },
        );

        logger.info(
          `[GitHub Push] Enqueued resync for context ${ctx.id} (${repoFullName}@${pushedBranch})`,
        );
        resynced++;
      }
      ...
      if (resynced === 0) {
        logger.info(
          `[GitHub Push] No contexts linked to ${repoFullName}@${pushedBranch} — nothing to resync`,
        );
      }
    } catch (err) {
      logger.error("[GitHub Push] Failed to enqueue context resync jobs", err);
      Sentry.captureException(err, {
        tags: { event: "github_push_resync", repo: repoFullName, branch: pushedBranch },
      });
    }

    // --- Step 2: Trigger GitNexus re-analysis (only if enabled) ---
    if (EnvUtils.get("USE_GITNEXUS", "") !== "true") return;
    ...
    try {
      ...
      if (!mcpUrl) {
        logger.warn("GITNEXUS_MCP_URL not configured. Skipping GitNexus re-analysis.");
        return;
      }

      const gitUrl = `https://x-access-token:${token}@github.com/${repoFullName}.git`;
      const analyzeUrl = mcpUrl.replace("/api/mcp", "/api/analyze");
      logger.info(`[GitHub Push] Triggering GitNexus re-analysis for ${repoFullName}`);

      const response = await fetch(analyzeUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: gitUrl, dropEmbeddings: true }),
        signal: AbortSignal.timeout(30000),
      });

      const data = await response.json();
      logger.info(`[GitHub Push] GitNexus job queued: ${data.jobId || data.status || "unknown"}`);
    } catch (err) {
      logger.error("[GitHub Push] Failed to trigger GitNexus re-analysis", err);
      Sentry.captureException(err, {
        tags: { event: "github_push_gitnexus", repo: repoFullName },
      });
    }
  }
}
