...
export class ChatController extends BaseWorkspaceController {
  ...
  private async resolveAndValidate(
    ...
    const fromProjects = projectIds && projectIds.length > 0
      ? await resolveProjectIdsToContextIds(projectIds)
      : [];
   ...
   */
  private async hydrateThreadResponse<
    T extends { contextIds: string[] },
  >(thread: T): Promise<T & { projectIds: string[] }> {
  ...
  public async listThreads(
    ...
    const hydrated = await Promise.all(
      threads.map((t) => this.hydrateThreadResponse(t)),
    );
