...
interface GenerateSingleSlideRequest {
  prompt: string;
  position: number;
  slideTypeKey?: string;
}
...
export class PresentationController extends BaseWorkspaceController {
  @Post("generate")
  ...
  @Post("{presentationId}/slides/generate")
  ...
  public async getPresentation(
    ...
    const { user, workspaceId } = await this.getAuthorizedWorkspaceAccess(request);
    const presentation = await slideGenerationService.getPresentationById(
      user.id,
      workspaceId,
      presentationId,
    );
  ...
  public async deletePresentation(
    ...
    const { user, workspaceId } = await this.getAuthorizedWorkspaceAccess(request);
    await slideGenerationService.deletePresentation(user.id, workspaceId, presentationId);
    return { success: true };
  }
  ...
  public async updatePresentation(
    ...
    const presentation = await slideGenerationService.updatePresentation(
      ...
      {
        ...body,
        slidesJson: body.slidesJson as Prisma.InputJsonValue,
      },
    );
    return this.mapPresentationResponse(presentation);
  }
    ...
    return {
      ...
      theme: presentation.theme
        ? {
            ...
            logoUrl: presentation.theme.logoUrl,
            ...
            backgroundStyle: presentation.theme.backgroundStyle,
            cardStyle: presentation.theme.cardStyle,
          }
        : undefined,
