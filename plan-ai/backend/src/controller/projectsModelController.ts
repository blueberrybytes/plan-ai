...
interface CreateTranscriptRequest {
  ...
  /** Meeting-type playbook outputs: generate a doc / slide deck from this meeting. */
  createDoc?: boolean;
...
export class ProjectsModelController extends BaseWorkspaceController {
  ...
  /**
   * Generate (or regenerate) the Project Digest: a living synthesis of ALL the
   ...
   */
  @Post("{projectId}/digest")
  @Security("ClientLevel")
  public async generateProjectDigest(
    @Request() request: AuthenticatedRequest,
    @Path() projectId: string,
  ): Promise<ApiResponse<{ digestDocId: string; title: string }>> {
    const { user, workspaceId } = await this.getPaidWorkspaceAccess(request);
    await this.getProjectForWorkspace(request, projectId, workspaceId);
    ...
    if (!result) {
      ...
      throw {
        status: 400,
        message: "No meetings available to summarize (none yet, or all are confidential).",
      };
    }

    return {
      status: 200,
      message: "Project digest generation started",
      data: result,
    };
  }
