...
export class UserController extends Controller {
  ...
  @Response<GenericResponse>(404, "Not Found")
  @Response<GenericResponse>(500, "Internal Server Error")
  public async deleteUser(@Path("userId") userId: string): Promise<ApiResponse<null>> {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
      });
      ...
      try {
        ...
        logger.warn(
          `Could not delete user ${user.firebaseUid} from Firebase. They might already be deleted.`,
          fbError,
        );
      }
