...
class Logger {
  ...
  /**
   * Forward a log record to Sentry. Never throws (logging must be safe).
   * Policy: ONLY logger.error reaches Sentry — warn stays log-only. If a
   * degradation deserves an alert, promote the call site to logger.error
   * instead of forwarding warns wholesale.
   */
    ...
    try {
      if (error instanceof Error) {
        Sentry.captureException(error, {
          level: "error",
          extra: { loggerMessage: message, ...(additional ?? {}) },
        });
      } else {
        Sentry.captureMessage(message, {
          level: "error",
          extra: {
            ...(error !== undefined ? this.toErrorFields(error) : {}),
            ...(additional ?? {}),
          },
        });
      }
    } catch {
      // Never let Sentry forwarding break the app.
    }
  }
  ...
  public error(
    ...
    this.captureToSentry(message, error, additional);
  }
