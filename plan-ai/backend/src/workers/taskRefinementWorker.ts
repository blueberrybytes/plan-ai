...
export const taskRefinementWorker = new Worker<TaskRefinementJobPayload>(
  ...
  async (job: Job<TaskRefinementJobPayload>) => {
    return Sentry.withScope(async (scope) => {
      ...
      try {
        await job.log(
          `[start] transcript=${job.data.transcriptId} tasks=${job.data.taskIds.length} contexts=${(job.data.contextIds ?? []).join(",")}`,
        );
        ...
        await projectTranscriptService.refineTasksWithInvestigation(job.data, onProgress);
        await job.log(`[done] job=${job.id} transcript=${job.data.transcriptId}`);
