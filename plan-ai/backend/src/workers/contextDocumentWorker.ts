  ...
  async (job: Job<ContextDocumentJobPayload>) => {
    ...
    return Sentry.withScope(async (scope) => {
          ...
          await prisma.contextFile.update({
            ...
            data: {
              metadata: {
                ...
                processingError: "Subscription required. Re-subscribe and retry the upload.",
                failureReason: "subscription_required",
              } as Prisma.InputJsonValue,
            },
          });
        } catch (markErr) {
          logger.error(`[worker:contextDoc] Failed to mark file ${fileId} as FAILED`, markErr);
        }
        return;
      }
      ...
      try {
        ...
        const buffer = await getContextFileContentFromFirebaseStorage(
          fileRecord.bucketPath,
          userId,
        );
...
${rawText.slice(0, 20000)}`;
        ...
        const keywordsPromise = generateText({
          ...
          output: Output.object({
            ...
            description:
              "Extracts unique keywords, entities, and acronyms from the document for future AI transcription boosting.",
            schema: z.object({
        ...
        let formattingPromise: Promise<Awaited<ReturnType<typeof generateText>> | null> | null =
          null;
          ...
          if (extractedKeywords.length > 0) {
            ...
            if (ctx) {
              ...
              const uniqueKeywords = Array.from(
                new Set([...currentKeywords, ...extractedKeywords]),
              );
