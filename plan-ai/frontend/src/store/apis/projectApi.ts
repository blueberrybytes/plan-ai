...
export type ApiResponseProjectDigest =
  components["schemas"]["ApiResponse__digestDocId-string--title-string__"];
...
export type ImportTranscriptRequest =
  operations["ImportProjectTranscript"]["requestBody"]["content"]["application/json"];
...
export type RefineTaskRequest =
  operations["RefineTask"]["requestBody"]["content"]["application/json"];
...
export const projectApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    generateProjectDigest: builder.mutation<ApiResponseProjectDigest, string>({
      query: (projectId) => ({
        url: `/api/projects/${projectId}/digest`,
        method: "POST",
      }),
    ...
    >({
      query: (arg: UploadProjectTranscriptArgs) => {
        ...
        if (complexityLevel) formData.append("complexityLevel", complexityLevel);
        ...
        if (agenticInvestigation !== undefined)
          formData.append("agenticInvestigation", agenticInvestigation.toString());
        if (createDoc !== undefined) formData.append("createDoc", createDoc.toString());
        if (createSlides !== undefined) formData.append("createSlides", createSlides.toString());
    ...
    importProjectTranscript: builder.mutation<
      ApiResponseCreateTranscriptResponse,
      { projectId: string; body: ImportTranscriptRequest }
    >({
      query: (arg: { projectId: string; body: ImportTranscriptRequest }) => ({
        url: `/api/projects/${arg.projectId}/transcripts/import`,
        method: "POST",
        body: arg.body,
      }),
      invalidatesTags: (result, error, arg) => [
    ...
    refineProjectTask: builder.mutation<
      ApiResponseRefineTaskResponse,
      { projectId: string; body: RefineTaskRequest }
    >({
      query: ({ projectId, body }) => ({
        url: `/api/projects/${projectId}/tasks/refine`,
        method: "POST",
        body,
      }),
    }),
