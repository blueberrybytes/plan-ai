...
function* loginMicrosoftFunc(): Generator<any, void, any> {
  try {
    ...
    const apiUrl = `${getApiBaseUrl()}/api/session/login`;
    ...
    try {
      ...
      // Sign out from Firebase to prevent half-login state where backend doesn't know the user
  ...
  try {
    ...
    try {
      ...
      if (response.data) {
        ...
        // Our backend returns { status: 200, data: UserResponse }
        // So we need response.data.data
        const backendPayload = (response.data as any).data || response.data;
        const userData = mapApiUserToUserType(backendPayload);
        yield put(setUserDb(userData));
...
function* resetAllApiStates(): Generator<any, void, any> {
  ...
  yield put(authApi.util.resetApiState());
}
