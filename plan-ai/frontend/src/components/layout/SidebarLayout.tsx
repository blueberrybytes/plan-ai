...
const SidebarLayout: React.FC<SidebarLayoutProps> = ({ children, fullHeight = false }) => {
  ...
  // Skip until a workspace is active: a subscription request without an
  // X-Workspace-Id header 400s, and RTK Query (keyed by endpoint, not header)
  // would not retry once the workspace is selected. Skipping makes it run
  // cleanly the moment activeWorkspaceId is set.
  const { data: subscription } = useGetSubscriptionQuery(undefined, { skip: !activeWorkspaceId });
  ...
  // Trial countdown shown in the workspace. During a trial Stripe's
  // current_period_end IS the trial-end date, which we already sync to the
  // workspace and expose as currentPeriodEnd — so no extra Stripe call needed.
  const trialDaysLeft =
              ...
              >
                ...
                >
                  <LinearProgress
                    variant="determinate"
                    value={
                      isByokTrack ? Math.min(100, (estimatedCost / 10) * 100) : tokenPercentage
                    }
