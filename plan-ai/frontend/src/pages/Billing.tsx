...
const Billing: React.FC = () => {
  ...
  const hasExistingKeys = Boolean(activeWorkspace?.openRouterKey && activeWorkspace?.deepgramKey);
  ...
  const handleSaveKeys = async () => {
    try {
      ...
      dispatch(
        setToastMessage({ message: "API keys configured successfully!", severity: "success" }),
      );
      setShowByokModal(false);
      dismissStatus();
    } catch (e) {
      console.error(e);
      // Surface the backend's specific validation message (e.g. which key is invalid).
      const message =
        (e as { data?: { message?: string } })?.data?.message || "Failed to save API keys";
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!byokModalShownRef.current && freshTrack === "BYOK" && !hasExistingKeys) {
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        dispatch(
          setToastMessage({ message: "Subscription updated successfully.", severity: "success" }),
        );
      } catch (err) {
        ...
        <Container maxWidth="sm" sx={{ py: 8 }}>
          <Stack spacing={4} alignItems="center" textAlign="center">
            ...
            <Typography variant="body1" color="text.secondary" sx={{ maxWidth: 420 }}>
              Your workspace has been granted complimentary access to all Plan AI features. No
                  ...
                  <Stack direction="row" spacing={1.5} alignItems="center" sx={{ mb: 1 }}>
                    ...
                    {subscription.status === "TRIALING" && subscription.currentPeriodEnd && (
                      <Chip
                        ...
                        color="info"
                        variant="outlined"
                        label={t("billing.trial.daysLeft", {
                          days: Math.max(
                            0,
                            Math.ceil(
              ...
              {catalog?.prices.map((entry) => (
                <Grid item xs={12} sm={6} md={3} key={entry.priceId}>
                  <CatalogCard
                    ...
                    byokTrialDays={catalog?.byokTrialDays ?? 0}
      ...
      <Dialog open={showByokModal} onClose={() => setShowByokModal(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography sx={{ mb: 3 }}>
            ...
            and insights, you must configure your API keys below.
          </Typography>
...
const CatalogCard: React.FC<{
  ...
  onManagePortal: () => void;
  loading: boolean;
}> = ({ entry, currentPriceId, byokTrialDays, onSelect, onManagePortal, loading }) => {
  const { t } = useTranslation();
  ...
  return (
    ...
    >
      ...
      {isByok && byokTrialDays > 0 && (
        <Typography variant="body2" sx={{ color: "secondary.main", fontWeight: 700, mt: -1 }}>
          {t("billing.trial.badge", { days: byokTrialDays })}
        </Typography>
      )}
      <Chip
      ...
      {isByok && (
        ...
        >
          {t("billing.requiresKeys", {
            defaultValue: "You must provide your own OpenRouter and Deepgram API keys.",
          })}
        </Alert>
      )}
