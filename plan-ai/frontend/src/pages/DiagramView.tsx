...
const DiagramView: React.FC = () => {
  ...
  const currentThemeObj = themes.find((t) => t.id === theme) || undefined;
  ...
  const handleDownloadPNG = async () => {
    ...
    try {
      ...
      const dataUrl = await toPng(node, {
        backgroundColor: currentThemeObj?.backgroundColor || "#ffffff",
  ...
  const handleDownloadSVG = () => {
    ...
    svgNode.style.backgroundColor = currentThemeObj?.backgroundColor || "#ffffff";
    ...
    bgRect.setAttribute("fill", currentThemeObj?.backgroundColor || "#ffffff");
              ...
              {sidebarTab === 1 && (
                <Box sx={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
                  ...
                  >
                    ...
                    {((diagram.status === "GENERATING" && chatMessages.length > 1) ||
                      isImproving) && (
            ...
            >
              {diagram?.status === "GENERATING" ? (
                ...
                >
                  ...
                  <Typography color="text.secondary">AI is constructing your diagram...</Typography>
                </Box>
              ) : code ? (
                <MermaidRenderer
                  chart={code}
                  theme={themes.find((t) => t.id === theme)}
                  isFixing={isImproving}
                  onErrorStateChange={(hasError) => {
                    // We only notify the console of error states rather than auto-fixing silently
                    if (hasError) {
                      console.log("[DiagramView] Mermaid renderer reported a syntax error.");
                    }
                  }}
                  onFixDiagram={async (errorMessage?: string) => {
                    if (!diagramId) return;
                    setIsModified(false);
                    setPreviousCode(code);
                    await improveDiagram({
                      id: diagramId,
                      body: {
                        instruction: `The diagram has syntax errors preventing it from rendering. Please carefully fix the Mermaid syntax so it is completely valid.\n\nPARSER ERROR:\n${errorMessage || "Unknown Syntax Error"}`,
                        currentCode: code,
                      },
                    });
                  }}
                ...
                >
                  <Typography color="text.secondary">No Mermaid syntax provided.</Typography>
                </Box>
              )}
            </Box>
          </Box>
