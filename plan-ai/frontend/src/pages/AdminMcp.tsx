...
const SseTester: React.FC = () => {
  ...
  const start = () => {
    ...
    void fetch(url, {
      ...
      if (!reader) { push("error", "No readable body"); setRunning(false); return; }
      const decoder = new TextDecoder();
      let buf = "";
      // eslint-disable-next-line no-constant-condition
      while (true) {
        const { done, value } = await reader.read();
        if (done) { push("closed", "Stream closed by server."); setRunning(false); break; }
          ...
          {logs.length === 0 ? (
            ...
            logs.map((l, i) => (
              <Box key={i} sx={{ display: "flex", gap: 1, mb: 0.3 }}>
                ...
                <Typography variant="caption" color={logColor[l.type]} sx={{ wordBreak: "break-all" }}>
                  {l.text}
                </Typography>
              </Box>
            ))
            ...
            <Stack spacing={1}>
              {[
                ...
                <Stack key={row.label} direction="row" spacing={2} alignItems="flex-start">
                  <Typography variant="caption" color="text.secondary" sx={{ minWidth: 110, flexShrink: 0, fontWeight: 600, pt: 0.4 }}>
                    {row.label}
                  </Typography>
                  <Box
                    component="code"
