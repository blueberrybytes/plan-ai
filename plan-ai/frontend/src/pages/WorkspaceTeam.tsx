...
const WorkspaceSettingsSection: React.FC<{ activeWorkspace: WorkspaceResponse }> = ({
  ...
  const handleSave = async () => {
    try {
      ...
      console.error(e);
      // Surface the backend's specific message (e.g. "Invalid API key: OpenRouter")
      // so the user knows exactly which key to fix.
      const message =
        (e as { data?: { message?: string } })?.data?.message || "Failed to update settings";
  ...
  return (
    ...
    >
      <Paper elevation={1} sx={{ p: 3, flex: 1, maxWidth: 600 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Used for AI-generated docs & slides when a meeting has no project theme (e.g. standalone
          recordings). Projects can override this.
        </Typography>
      ...
      <Box sx={{ p: { xs: 2, md: 4 } }}>
        ...
        {isLoading ? (
                  ...
                  <TableHead sx={{ bgcolor: "background.default" }}>
                    <TableRow>
                      ...
                      <TableCell align="right">
                        {isAdmin ? "Blueberry Tokens" : "Usage Units"}
                      </TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
          ...
          >
            ...
            <Typography variant="body2" color="text.secondary" paragraph>
              Your current subscription allows for <b>{totalSeats}</b> seats. You have filled{" "}
              <b>{usedSeats}</b> of them.
            </Typography>
            ...
            <Typography variant="body2" color="text.secondary" paragraph>
              You can increase your seat limit at any time. When you add seats, your billing will be
