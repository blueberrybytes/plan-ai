...
const SlideCreate: React.FC = () => {
  ...
  const handleGenerate = async () => {
    if (!prompt.trim() && selectedProjectIds.length === 0 && selectedTranscriptIds.length === 0)
      return;
    try {
      setError(null);
        ...
        >
          ...
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                {...params}
                label={t("docs.create.transcriptsLabel", "Select Recordings")}
                helperText="Include transcripts as context"
              />
            )}
            sx={{ mb: 3 }}
          />
