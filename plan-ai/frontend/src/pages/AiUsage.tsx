...
import { useGetMyWorkspacesQuery, useGetWorkspaceMembersQuery } from "../store/apis/workspaceApi";
...
export const AiUsageContent: React.FC<{ hideBreadcrumbs?: boolean }> = ({
  ...
  return (
    <Box sx={hideBreadcrumbs ? { width: "100%" } : { p: 4, margin: "0 auto", width: "100%" }}>
      {!hideBreadcrumbs && (
                  ...
                  <Box sx={{ mb: 2 }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
                      ...
                      <Typography
                        ...
                        sx={{
                          fontWeight: 600,
                          color: usageLimits.llm.percentage >= 90 ? "error.main" : "text.primary",
                        }}
                      >
                        {usageLimits.llm.used.toLocaleString()} /{" "}
                        {usageLimits.llm.allowed.toLocaleString()} ({usageLimits.llm.percentage}%)
                      </Typography>
                    </Box>
                    <LinearProgress
                      variant="determinate"
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
                      ...
                      <Typography
                        ...
                        sx={{
                          fontWeight: 600,
                          color:
                            usageLimits.recording.percentage >= 90 ? "error.main" : "text.primary",
                        }}
                      >
                        {(usageLimits.recording.used / 60).toFixed(1)} /{" "}
                        {(usageLimits.recording.allowed / 60).toFixed(0)} hrs (
                        {usageLimits.recording.percentage}%)
                      </Typography>
                    </Box>
                    <LinearProgress
                      variant="determinate"
                  ...
                  <Box>
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
                      <Typography variant="body2">
                        Generations (Docs / Slides / Diagrams)
                      </Typography>
                      <Typography
                        variant="body2"
                        sx={{
                          fontWeight: 600,
                          color:
                            usageLimits.generations.percentage >= 90
                              ? "error.main"
                              : "text.primary",
                        }}
                      >
                        {usageLimits.generations.used} / {usageLimits.generations.allowed} (
                        {usageLimits.generations.percentage}%)
                      </Typography>
                    </Box>
                    <LinearProgress
                      variant="determinate"
